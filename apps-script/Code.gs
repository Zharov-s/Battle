const CONFIG = {
  ...
  WEEKLY_BASELINES: { grand_city: 407021.80, riviera_city: 833268.57 },
  HOTEL_PRICE: 100000,
...
function weeklyTeam_(key,name,rows){const goal=CONFIG.WEEKLY_MISSIONS[key];const parsed=rows.map(r=>({row:r,date:r.date instanceof Date?r.date:new Date(r.date)}));const hasDates=parsed.some(x=>x.date instanceof Date&&!isNaN(x.date));const dated=parsed.filter(x=>x.date instanceof Date&&!isNaN(x.date)&&x.date>=CONFIG.WEEKLY_START&&x.date<=CONFIG.WEEKLY_END&&x.row.manager&&x.row.amount>0).map(x=>x.row);const currentTotal=rows.filter(r=>r.manager&&r.amount>0).reduce((s,r)=>s+r.amount,0);const fact=hasDates?dated.reduce((s,r)=>s+r.amount,0):Math.max(currentTotal-CONFIG.WEEKLY_BASELINES[key],0);return {team_key:key,team_name:name,mission_amount:goal,fact_amount:fact,remaining_amount:Math.max(goal-fact,0),progress_percent:goal?fact/goal*100:0};}
