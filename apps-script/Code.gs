const CONFIG = {
  ...
  WEEKLY_START_ROWS: { grand_city: 20, riviera_city: 36 },
  HOTEL_PRICE: 100000,
...
function buildPayload_(){
  ...
  return {meta:{game_title:'Игры престолов. Битва городов',subtitle:'Игровая механика для команд подключений',period_start:'2026-07-20',period_end:'2026-08-31',timezone:'Europe/Moscow',hotel_price:CONFIG.HOTEL_PRICE,generated_at:new Date().toISOString()},weekly_mission:{period_start:'2026-07-27',period_end:'2026-07-31',teams:[weeklyTeam_('grand_city','Команда Шкильнюка',gra),weeklyTeam_('riviera_city','Команда Китаевой',riv)]},teams:teams,overall:{total_mission:totalMission,total_fact:totalFact,total_progress_percent:totalFact/totalMission*100,total_remaining:Math.max(totalMission-totalFact,0),leader_team_key:leader?leader.team_key:null,leader_label:leader?leader.city_name:'Равенство'},manager_top:managerTop,product_distribution:productDistribution,data_quality:{status:errors?'warning':'ok',error_count:errors,warnings:errors?['Проверьте даты, ФИО, суммы и продукты.']:[]}};
}
...
function valid_(r,upsell){if(upsell)return r.manager&&r.amount>0;const d=parseDate_(r.date);return d&&d>=CONFIG.PERIOD_START&&d<=CONFIG.PERIOD_END&&r.manager&&r.amount>0;}
...
function weeklyTeam_(key,name,rows){const goal=CONFIG.WEEKLY_MISSIONS[key];const hasDates=rows.some(r=>parseDate_(r.date));const startIndex=Math.max((CONFIG.WEEKLY_START_ROWS[key]||2)-2,0);const source=hasDates?rows:rows.slice(startIndex);const valid=source.filter(r=>r.manager&&r.amount>0);const fact=(hasDates?valid.filter(r=>{const d=parseDate_(r.date);return d&&d>=CONFIG.WEEKLY_START&&d<=CONFIG.WEEKLY_END;}):valid).reduce((s,r)=>s+r.amount,0);return {team_key:key,team_name:name,mission_amount:goal,fact_amount:fact,remaining_amount:Math.max(goal-fact,0),progress_percent:goal?fact/goal*100:0};}
