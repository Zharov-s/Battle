...
function valid_(r,upsell){const d=r.date instanceof Date?r.date:new Date(r.date);const allowed=['МКБ','МОБ','РКЛ','Проверка гостей','iiko'];return d instanceof Date&&!isNaN(d)&&d>=CONFIG.PERIOD_START&&d<=CONFIG.PERIOD_END&&r.manager&&r.amount>0&&(!upsell||allowed.indexOf(r.product)>-1);}
