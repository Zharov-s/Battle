(() => {
  ...
  function weeklyTeam(key,name,rows){
    ...
    const eligible=rows.filter(r=>(!hasDates||inWeeklyPeriod(parseDate(r.date)))&&r.manager.trim()&&r.amount>0);
    const fact=eligible.reduce((sum,r)=>sum+r.amount,0);
  ...
  function buildPayload(rivRows,graRows,upRows,rivMission,graMission){
    ...
    const weeklyFallback=[graRows,rivRows].some(rows=>rows.some(r=>r.manager.trim()&&r.amount>0)&&!rows.some(r=>parseDate(r.date)));
    const warnings=[]; if(weeklyFallback) warnings.push('В командном листе нет корректных дат: недельный факт рассчитан по всем заполненным продажам.');
    return {meta:{game_title:'Sales Kombat',subtitle:'Игровая механика для команд подключений',period_start:cfg.PERIOD_START,period_end:cfg.PERIOD_END,hotel_price:cfg.HOTEL_PRICE,generated_at:new Date().toISOString()},weekly_mission:{period_start:cfg.WEEKLY_START,period_end:cfg.WEEKLY_END,teams:[weeklyTeam('grand_city','Команда Шкильнюка',graRows),weeklyTeam('riviera_city','Команда Китаевой',rivRows)]},teams,overall:{total_mission:totalMission,total_fact:totalFact,total_progress_percent:totalFact/totalMission*100,total_remaining:Math.max(totalMission-totalFact,0),leader_team_key:leader?.team_key||null,leader_label:leader?.city_name||'Равенство'},manager_top,product_distribution,data_quality:{status:(errors||legacyNoDates||weeklyFallback)?'warning':'ok',error_count:errors,warnings:warnings.concat(errors?[`Строк вне периода или с незаполненными обязательными полями: ${errors}`]:[])}};
  }
  ...
  function render(data){
    ...
    document.querySelector('.champion-personal img').src='./assets/champion-personal.png?v=1';
