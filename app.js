(() => {
  ...
  function versus(data){
    ...
    const gShare=sum?gp/sum*100:50, rShare=100-gShare;
    const lead=data.overall.leader_team_key, gLead=lead==='grand_city', rLead=lead==='riviera_city';
    const pct=v=>`${v.toFixed(1).replace('.',',')}%`;
    const gap=Math.abs(gp-rp);
    const status=gLead?`${g.city_name} впереди на ${pct(gap)}`:rLead?`${r.city_name} впереди на ${pct(gap)}`:'Равная борьба';
    const side=(t,kicker,leadFlag)=>`<div class="vs-side ${t.team_key==='grand_city'?'grand':'riviera'} ${leadFlag?'lead':''}"><div class="vs-flag">⚑</div><div class="vs-kicker">${esc(kicker)}</div><div class="vs-city">${esc(t.city_name)}</div><div class="vs-score">${pct(t.progress_percent)}</div><div class="vs-fact">${money(t.fact_amount)} из ${money(t.mission_amount)}</div><span class="vs-pill">${leadFlag?'Лидер':'В борьбе'}</span></div>`;
    return `<section class="versus card ${gLead?'grand-lead':rLead?'riviera-lead':'tie'}"><div class="versus-grid">${side(g,'Команда Шкильнюка',gLead)}<div class="vs-center"><div class="vs-badge"><b>VS</b></div><div class="vs-status">${esc(status)}</div></div>${side(r,'Команда Китаевой',rLead)}</div><div class="tug"><div class="tug-track"><div class="tug-fill grand" style="width:${gShare}%"></div><div class="tug-fill riviera" style="width:${rShare}%"></div><div class="tug-mid"></div><div class="tug-knob" style="left:${gShare}%"></div></div><div class="tug-legend"><span class="grand">${esc(g.city_name)}</span><span class="small">Перетягивание по проценту выполнения миссии</span><span class="riviera">${esc(r.city_name)}</span></div></div></section>`;
  }
