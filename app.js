(() => {
  ...
  function marketComparison(data){
    ...
    const outcome=(t,owner,leadFlag)=>{
      const cls=t.team_key==='grand_city'?'grand':'riviera';
      return `<article class="market-outcome ${cls} ${leadFlag?'lead':''}"><div class="market-outcome-head"><div><span>${esc(owner)}</span><h3>${esc(t.city_name)}</h3></div><b>${leadFlag?'Лидер':'В борьбе'}</b></div><div class="market-score">${pct(t.progress_percent)}</div><div class="market-money"><strong>${money(t.fact_amount)}</strong><span>из ${money(t.mission_amount)}</span></div><div class="market-progress" role="progressbar" aria-label="Выполнение миссии ${esc(t.city_name)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.min(Math.round(t.progress_percent),100)}"><i style="width:${Math.min(t.progress_percent,100)}%"></i></div></article>`;
    };
    return `<section class="market card"><header class="market-head"><div class="market-tags"><span class="market-live"><i></i>В реальном времени</span><span>Миссия месяца</span></div><span>Обновление каждые 5 минут</span></header><div class="market-question"><div><div class="eyebrow">Текущий прогноз</div><h2>Кто ближе к выполнению миссии?</h2></div><div class="market-leader"><span>${leader?'Сейчас впереди':'Равная борьба'}</span><strong>${leader?esc(leader.city_name):'Ничья'}</strong>${leader?`<small>разрыв ${pct(gap)}</small>`:''}</div></div><div class="market-stats"><div><span>Общий факт</span><strong>${money(data.overall.total_fact)}</strong></div><div><span>Шкильнюк</span><strong>${money(g.fact_amount)}</strong></div><div><span>Китаева</span><strong>${money(r.fact_amount)}</strong></div></div><div class="market-outcomes">${outcome(g,'Команда Шкильнюка',gLead)}${outcome(r,'Команда Китаевой',rLead)}</div></section>`;
  }
